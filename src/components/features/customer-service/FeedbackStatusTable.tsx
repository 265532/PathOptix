...
const FeedbackStatusTable: React.FC = () => {
  ...
  return (
    <div className="bg-bg-tertiary rounded-[32px] border border-border-default p-8 shadow-2xl h-full flex flex-col">
          ...
          <tbody className="divide-y divide-border-default/30">
            {data.map((item, idx) => (
              <tr key={idx} className="group hover:bg-bg-tertiary/10 transition-colors duration-300">
                <td className="py-6 font-mono text-xs text-text-muted font-black">{item.id}</td>
                <td className="py-6">
                ...
                <td className="py-6 text-[11px] text-text-muted max-w-[140px] truncate">{item.desc}</td>
