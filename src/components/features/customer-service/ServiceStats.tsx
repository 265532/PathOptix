...
const StatBox = ({ icon, label, value, trend, color }: any) => (
  <div className="bg-bg-secondary border border-border-default rounded-3xl p-6 hover:border-border-input transition-all duration-300 group overflow-hidden relative">
    <div className="flex justify-between items-start mb-4">
      <div className={`p-3 rounded-2xl bg-bg-primary border border-border-default ${color}`}>
        {icon}
      </div>
      <span className={`text-[10px] font-black px-2 py-0.5 rounded bg-bg-primary border border-border-default ${trend.startsWith('+') ? 'text-emerald-400' : 'text-amber-400'}`}>
        {trend}
      </span>
    </div>
