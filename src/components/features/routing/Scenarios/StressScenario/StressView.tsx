...
import { simulationApi, RlPathJson } from '@/services';
...
const StressView: React.FC<StressViewProps> = ({ rlData, startLabel = '上海', endLabel = '鹿特丹' }) => {
  const [simData, setSimData] = useState<SimulationRunResponse | null>(null);
  const [isSimulating, setIsSimulating] = useState(false);
  const [showCompleteModal, setShowCompleteModal] = useState(false);

  // 获取 RL 基础数据
  const rlCost = rlData?.total_cost_usd ?? 5000;
  const rlTime = rlData?.total_time_days ?? 20;
  const rlCarbon = rlData?.total_carbon_kg ?? 200;
